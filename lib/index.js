 ...
 */
function imageMediaType(ref) {
    const value = ref?.mediaType;
    return typeof value === 'string' && value.length > 0 ? value : undefined;
}
...
async function imageParts(images, readImage) {
    const parts = [];
...
async function messagesToCompletions(messages, readImage, allowImages) {
    ...
    const flushToolImages = async () => {
        if (pendingToolImages.length === 0 || toolImageReader === undefined)
            return;
...
export function apply(ctx, config) {
    ...
    async function* stream(options) {
        ...
        try {
            ...
            if (headTimedOut) {
                throw new LlmError(`Hyper API request did not respond within ${cfg.requestTimeoutMs}ms: ${error instanceof Error ? error.message : String(error)}；Hyper API 请求在 ${cfg.requestTimeoutMs} 毫秒内未返回响应——通常是网络或代理问题，请检查后重试`, 'TIMEOUT', { cause: error });
            }
            if (options.signal?.aborted === true)
                throw error;
            throw new LlmError(`Hyper API request failed: ${error instanceof Error ? error.message : String(error)}；Hyper API 请求发送失败——通常是网络或代理问题，请检查网络或代理设置后重试`, 'TRANSPORT', { cause: error });
        }
        finally {
