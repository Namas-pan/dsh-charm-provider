window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const STYLE_ID = "dsh-charm-provider-styles";
    ...
    const CSS = [
      ...
      ".hyper-balance{font-size:26px;font-weight:600;line-height:1.15;font-variant-numeric:tabular-nums}",
      ".hyper-balance-sm{font-size:14px;font-weight:600;font-variant-numeric:tabular-nums}",
    ].join("\n");
    ...
    function HyperProviderCard(props) {
      ...
      const [credits] = useCredits(faces);
      const view = credits.view;
      return react.createElement("div", { className: "hyper-provider" },
        ...
        react.createElement("div", { className: "hyper-row" },
          react.createElement("span", { className: "hyper-note" }, "Hypercredits"),
          react.createElement("span", { className: "hyper-balance-sm" }, formatCredits(view === null ? null : view.balance)),
          view !== null && view.lastCostUsd !== null
            ? react.createElement("span", { className: "hyper-note" }, "· 最近一次 " + formatUsd(view.lastCostUsd))
            : null,
          credits.error === null ? null : react.createElement("span", { className: "hyper-err" }, credits.error),
        ),
    ...
    function HyperSettingsPage(props) {
      ...
      return react.createElement("section", { className: "hyper-page" },
        ...
        react.createElement(CreditsCard, { faces }),
        ...
        react.createElement("div", { className: "hyper-card" },
          ...
          react.createElement("p", { className: "hyper-note" },
            "用量与花费：Hyper 在响应里回传 cost.usd / cost.hypercredits 与剩余 hypercredits，上面的余额卡片优先用它（比 /v1/credits 的整数更精确）；执行 /hyper 可看文字版汇总与模型清单（/hyper models）。",
          ),
        ),
      );
    }
    ...
    /** Local validator for the credits result, mirroring the Host's. */
    function parseCreditsView(value) {
      if (value === null || typeof value !== "object") throw new Error("credits: expected an object");
      const numberOrNull = (key) => {
        const raw = value[key];
        if (raw === null) return null;
        if (typeof raw !== "number" || !Number.isFinite(raw)) throw new Error("credits." + key + ": expected a finite number");
        return raw;
      };
      return {
        balance: numberOrNull("balance"),
        source: typeof value.source === "string" ? value.source : "none",
        estimated: value.estimated === true,
        updatedAt: numberOrNull("updatedAt"),
        requests: numberOrNull("requests") ?? 0,
        spentUsd: numberOrNull("spentUsd") ?? 0,
        spentCredits: numberOrNull("spentCredits") ?? 0,
        lastCostUsd: numberOrNull("lastCostUsd"),
        lastModel: typeof value.lastModel === "string" ? value.lastModel : null,
        error: typeof value.error === "string" ? value.error : null,
      };
    }

    /** Same descriptor the Host registers; mounting it makes the call callable. */
    ...
    function creditsFaces(remote) {
      return {
        async read() {
          try {
            const response = await remote.credits();
            if (!response || response.ok !== true) return { ok: false, error: messageOf(response) };
            return { ok: true, view: parseCreditsView(response.value) };
          } catch (error) {
            return { ok: false, error: String((error && error.message) || error) };
          }
        },
    ...
    /** Credits with the precision Hyper actually reports, trailing zeros removed. */
    ...
    /** Load the credit state once per mount, with an explicit refresh. */
    function useCredits(faces) {
      const [state, setState] = react.useState({ status: "loading", view: null, error: null });
      const load = react.useCallback(() => {
        let live = true;
        setState((previous) => ({ ...previous, status: previous.view === null ? "loading" : "refreshing" }));
        faces.credits.read().then((result) => {
          if (!live) return;
          if (result.ok) setState({ status: "ready", view: result.view, error: result.view.error });
          else setState((previous) => ({ status: "error", view: previous.view, error: result.error }));
        });
        return () => { live = false; };
      }, [faces]);
    ...
    /** The credits card: balance, session spend, and the last request. */
    function CreditsCard(props) {
      const faces = props.faces;
      const [state, reload] = useCredits(faces);
      const view = state.view;
      const sourceLabel = view === null
        ? ""
        : view.source === "endpoint" ? (view.estimated ? "来自 /v1/credits，已扣除其后的花费" : "来自 /v1/credits")
          : view.source === "response" ? "来自上一次请求的 usage"
            : "尚无读数";
      const when = view !== null && view.updatedAt !== null ? new Date(view.updatedAt).toLocaleTimeString() : null;

      return react.createElement("div", { className: "hyper-card" },
        react.createElement("div", { className: "hyper-row" },
          react.createElement("h3", null, "Hypercredits 余额"),
          react.createElement("button", {
            className: "hyper-btn",
        ...
        react.createElement("div", { className: "hyper-balance" }, formatCredits(view === null ? null : view.balance)),
        react.createElement("p", { className: "hyper-note" },
          sourceLabel + (when === null ? "" : " · " + when),
        ),
        view === null || view.requests === 0
          ? null
          : react.createElement("dl", { className: "hyper-kv" },
            react.createElement("dt", null, "本次会话"),
            react.createElement("dd", null, view.requests + " 次请求 · " + formatUsd(view.spentUsd) + " · " + formatCredits(view.spentCredits) + " 积分"),
            react.createElement("dt", null, "最近一次"),
            react.createElement("dd", null, (view.lastModel === null ? "—" : view.lastModel) + " · " + formatUsd(view.lastCostUsd)),
          ),
        state.error === null ? null : react.createElement("p", { className: "hyper-err" }, "读取余额失败：" + state.error),
      );
    }
    ...
    exports.apply = function (ctx) {
      ...
      // Mount the credits Remote first: `remote.hyper` only exists afterwards,
      // so both seats wait for it (and the page says so if it never lands).
      ctx.effect(() => {
        let cancelled = false;
        let unmount;
        ctx.remote.$mount({ package: REMOTE_PACKAGE, descriptors: [CREDITS_DESCRIPTOR] }).then((dispose) => {
          if (cancelled) dispose();
          else unmount = dispose;
        }).catch(() => { /* no card data; the rest of the page still works */ });
        return () => {
          cancelled = true;
          if (unmount !== undefined) unmount();
        };
      }, "dsh-charm-provider: credits remote");
      ...
      ctx.inject(["remote.credentials", "remote.hyper"], (remoteCtx) => {
        const scope = ctx.settingsScope.bind({ namespace: SETTINGS_NS });
        const faces = {
          credentials: credentialFaces(remoteCtx.remote.credentials),
          credits: creditsFaces(remoteCtx.remote.hyper),
          scope,
          ref: CREDENTIAL_REF,
          ns: SETTINGS_NS,
        };
