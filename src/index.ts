...
/**
 * The media type of one `ImageAttachmentRef`, or `undefined` when it carries none.
 *
 * `AttachmentStore.readImage` answers `{ ref, data }` — the verified media type is
 * metadata on the returned reference, NOT a sibling of the bytes. Reading it off
 * the payload yields `data:undefined;base64,…`, which OpenAI-compatible providers
 * drop without raising, so the image silently never reaches the model.
 */
function imageMediaType(ref: unknown): string | undefined {
  const value = (ref as { mediaType?: unknown } | undefined)?.mediaType
  return typeof value === 'string' && value.length > 0 ? value : undefined
}
...
/** Leading text of the user message that carries a tool result's images. */
const TOOL_RESULT_IMAGE_TEXT = 'Attached image(s) from tool result:'

/** The `image` blocks of one content array, in order. */
function imageBlocksOf(blocks: unknown[]): unknown[] {
  return blocks.filter(b => (b as { type?: unknown }).type === 'image')
}

/** Embed ordered image blocks as OpenAI `image_url` data URLs. */
async function imageParts(images: unknown[], readImage: ReadImage): Promise<WirePart[]> {
...
async function userContent(blocks: unknown[], readImage: ReadImage | undefined, allowImages: boolean): Promise<string | WirePart[] | undefined> {
  ...
  parts.push(...await imageParts(images, readImage))
  return parts
}
 ...
 *
 * Tool messages stay STRING-ONLY — every OpenAI-compatible server accepts that —
 * so a run of tool results' images follows the run as ONE user message, the shape
 * the shipped DeepSeek adapter uses. Inline image parts inside a tool message, or
 * a user message wedged between two responses to the same assistant turn, are what
 * stricter servers reject.
 */
async function messagesToCompletions(messages: readonly unknown[], readImage: ReadImage | undefined, allowImages: boolean): Promise<WireMessage[]> {
  ...
  const toolImageReader = allowImages ? readImage : undefined
  const pendingToolImages: unknown[] = []
  const flushToolImages = async (): Promise<void> => {
    if (pendingToolImages.length === 0 || toolImageReader === undefined) return
    const images = pendingToolImages.splice(0, pendingToolImages.length)
    out.push({
      role: 'user',
      content: [{ type: 'text', text: TOOL_RESULT_IMAGE_TEXT }, ...await imageParts(images, toolImageReader)],
    })
  }
  for (const message of messages) {
    ...
    if (role === 'user' && source?.kind === 'tool') {
      ...
      const images = imageBlocksOf(inner)
      const omitted = images.length > 0 && toolImageReader === undefined
      const text = inner.map(blockText).filter(Boolean).join('\n')
      const body = omitted
        ? text.length > 0 ? `${text}\n(image omitted: this model accepts text only)` : '(image omitted: this model accepts text only)'
        : text.length > 0 ? text : '(no output)'
      out.push({ role: 'tool', tool_call_id: String(block.toolCallId ?? ''), content: body })
      // Deferred: the images ride one user message after this whole run of
      // tool results, never wedged between two responses of the same turn.
      if (!omitted) pendingToolImages.push(...images)
    }
  }
  await flushToolImages()
  return out
  ...
  async function* stream(options: GenerateOptions): AsyncGenerator<StreamChunk> {
    ...
    const readImage: ReadImage | undefined = attachments === undefined
      ...
      : async (ref) => {
        ...
        const mediaType = imageMediaType(stored.ref) ?? imageMediaType(ref)
        if (mediaType === undefined) {
          throw new LlmError('Hyper image input could not determine an attachment media type', 'UNSUPPORTED_CONTENT')
        }
        return { data: stored.data, mediaType }
      }
    ...
    // Two independent timeout layers, deliberately separated:
    //  - the *head-of-request* budget below covers only the wait for response
    //    headers, and is cleared the moment they arrive, so a slow-but-healthy
    //    generation is never killed by it;
    //  - the body stream is governed by the caller signal and the per-read idle
    //    watchdog (`streamIdleTimeoutMs`), never by a whole-response wall clock.
    const headAbort = new AbortController()
    let headTimedOut = false
    const headTimer = setTimeout(() => {
      headTimedOut = true
      const reason = new Error(`Hyper API request did not respond within ${cfg.requestTimeoutMs}ms`)
      reason.name = 'TimeoutError'
      headAbort.abort(reason)
    ...
    try {
      response = await fetch(`${cfg.baseURL}/chat/completions`, {
        ...
        signal: options.signal === undefined
          ? headAbort.signal
          : AbortSignal.any([options.signal, headAbort.signal]),
      })
    } catch (error) {
      if (headTimedOut) {
        throw new LlmError(
          `Hyper API request did not respond within ${cfg.requestTimeoutMs}ms: ${error instanceof Error ? error.message : String(error)}；Hyper API 请求在 ${cfg.requestTimeoutMs} 毫秒内未返回响应——通常是网络或代理问题，请检查后重试`,
          'TIMEOUT',
          { cause: error },
        )
      }
      ...
      throw new LlmError(
        `Hyper API request failed: ${error instanceof Error ? error.message : String(error)}；Hyper API 请求发送失败——通常是网络或代理问题，请检查网络或代理设置后重试`,
        'TRANSPORT',
        { cause: error },
      )
    } finally {
